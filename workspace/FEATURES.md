...
| P-19 | The app at phone width | app | Found at F4's close-out, from before F4: on a 375px screen the top bar (workspace selector and account links) reaches 597px, so every page scrolls sideways, and the Settings tab row does not wrap. Dialogs are unaffected (F4 fixed them to the window). Making the tab row scroll clips the active tab's underline, which overlaps the row's border, so it wants a small design, not a class. **Also at desktop width** (found in F8, 2026-09-26): with nine tabs since Collaboration joined the row, "Roles & permissions" wraps onto two lines at 1440px, so the tab row wants the same design at every width |
