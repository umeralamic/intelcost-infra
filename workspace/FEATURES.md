...
| F8 (P-07) | Realtime: socket, auth, Redis fan-out, collaboration modes, live drawing, F3 and F4 events | both | One socket per tab to `/api/realtime`, token in the first frame, re-authorised on refresh and never rebuilt · workspace and project topics, membership checked on join and on re-auth · events after commit (D-20) through Redis to every api process, and from Celery workers · write tokens suppress a tab's own echo · reconnect refetches, no replay · the collaboration mode (D-32): Work together by default, Warn me, One at a time with a 15 s Redis claim, refused 409 by the api · concurrent adds to one item safe under an item-row lock, same-shape conflicts named ("Sara W. just changed this shape") · saved items and shapes live on today's takeoff page (S18) · live in-progress drawing drawn with a "Sara W." tag (D-33, **D-34 pending review**), cursors carried (drawn by F7), five Collaboration preferences on the account · the F3 and F4 events each mapped to their own queries; a removed member told, moved on and revoked on every api process · the 13 legacy channels mapped → [PARITY.md §10](docs/PARITY.md#10-canvas-interactions) **+9 lines** (the soft-lock as One at a time, write tokens, seven of eight collaboration lines; cursors partial for F7), [§24](docs/PARITY.md#24-app-wide) **+1** (Collaboration preferences), §3's live line now live, the realtime table's "New event" column | **app:** `core/realtime/{socket,frames,tab,topics,provider,reconnecting-indicator,use-realtime-topic,use-realtime-status,use-item-presence,use-drafts,use-settled-refetch}.ts(x)`, `core/auth/use-collaboration-prefs.ts`, `features/{workspace,project}/realtime.ts`, `components/realtime-events.tsx`, `features/takeoff/hooks/useLiveItems.ts`, `features/takeoff/components/DraftLayer.tsx`, `features/auth/components/CollaborationPrefsSection.tsx`, `pages/SettingsCollaboration.tsx` · **api:** `features/realtime/{routes,connection,hub,publish,presence,service,schemas,context}.py`, `core/{names,outbox}.py`, the publishes in `features/{workspace,project,takeoff}/routes.py` and `worker/tasks/maintenance.py`, 2 migrations `e7b2c5a90d14`, `f3a8d2c61b57` · **bench:** `api-b` and `app-b` on the `realtime` profile, `intelcost-infra/browser/f8-*.mjs` and runners, `lib/realtime.mjs`, `lib/takeoff.mjs`, `drives/f8-bench.py`, `notes/2026-09-26_realtime_caddy_for_abdullah.md` · **spec:** [realtime_tasks.md](docs/archive/realtime_tasks.md) |
| P-19 | The app at phone width, and the settings tab row at every width | app | At 375 px the top bar folds to essentials (tighter gutters, the workspace name truncates, account and sign-out keep their icons and accessible names) and no page scrolls sideways; the settings tab row never wraps, scrolls on a narrow window with the active tab brought into view, and draws its rule as an inset shadow so the active underline is never clipped; all nine tabs fit on one line at 1440; activity lines wrap a long address. Measured before: top bar 222 px over at 375, pages up to 474 px sideways, the row 57 px over at 1440 | **app:** `components/{app-shell,settings-layout}.tsx`, `features/workspace/components/ActivityFeed.tsx`, `index.css` (`scrollbar-none`) · **bench:** `intelcost-infra/browser/p19.mjs` (in `regress.sh`) · **spec:** this row (overnight 2026-09-26; no separate spec) |
| P-18 | Route-level code splitting | app | Every page but Login, Dashboard and NotFound is its own chunk, fetched on the first visit to its route, through `core/boot/lazy-page.ts`, which sends a failed import of a deleted chunk to the stale-chunk guard (F2-S13). Pages inside the shell load under the top bar, which never blinks out. **Measured 2026-09-26:** the main chunk 664.74 kB → 231.87 kB; what the dashboard downloads on first load (the entry and its three preloaded shared chunks) 664.74 kB → 454.72 kB, gzip 196.43 → about 141 kB. Takeoff (55.74 kB) and Project Home (70.40 kB) load only on their routes; TipTap stays lazy (394 kB). F5 adds pdf.js and the canvas in their own chunks (F5-S3) | **app:** `App.tsx`, `core/boot/{lazy-page,stale-chunk}.ts`, `components/app-shell.tsx` · **bench:** `f2-s10`, `f2-s12`, `f2-s14` wait for a lazy page to draw; `drives/{f3-s13,f4-s2}-bundle.sh` still pass · **spec:** this row (overnight 2026-09-26) |
