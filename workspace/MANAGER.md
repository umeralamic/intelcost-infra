...
| F6 | An item today is only its shapes: no height, pitch, sub-items, variables or classification, and F3 left the classification block here. → The New Measurement and Properties dialog, height and pitch modifiers, named dimensions, sub-items with a formula engine run in both the browser and the api (kept equal by one shared table), workspace variables, folders with multipliers, the three seeded layers, the item tree, and F3-S14 to S18 (systems, the tree, archive, the CSI seed, subcontractors). Every item write keeps F8-S9's lock and the collaboration mode. **Specced overnight 2026-09-26.** | Be Fe | The founder's answers to Q1 to Q9; and F5, whose canvas and panel it sits on | [item_model_tasks.md](docs/tasks/item_model_tasks.md) |
